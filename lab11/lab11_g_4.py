stations = {"Acton Town" : ["Ealing Common","South Ealing","Turnham Green","Chiswick Park"],
...
def dijkstra(graph:AdjMatrix, src:int, dst:int):
...
def get_key(val:int) -> str:
    global vertexTable
    for key, value in vertexTable.items():
        if val == value:
            return key
    return None
vertexTable = dict()
for station in stations.keys():
    for neighbor in stations[station]:
        vertexTable.setdefault(station, len(vertexTable))
        vertexTable.setdefault(neighbor, len(vertexTable))
Matrix = AdjMatrix(len(vertexTable))
for station in stations.keys():
    for neighbor in stations[station]:
        Matrix.addEdge(vertexTable[station], vertexTable[neighbor], 1)
src, dst = input("Enter : ").split(',')
print(f"From: {src}")
print(f"To: {dst}")
print("Searching shortest route... This may take a while...")
print(f"Suggested Route: ")
print(list(map(get_key, dijkstra(Matrix, vertexTable[src], vertexTable[dst]))))